  ...
  * *Updated (August 2026 - Voice, NLU, Overlay, Reminders & YouTube Enhancements)*:
    ...
    - **Voice Assistant Core & Safety Overhaul (August 2026)**:
      ...
      - **Natural LLM Conversation Prompt & Expanded Search Routing Overhaul (September 2026)**:
        * LLM Model Cache & Reload Bug Diagnosed: When changing custom GGUF models (e.g. between Q2 and Q4) in `MainActivity`, `FridayService.reloadModels()` was an empty stub, and `AgentCore.kt` checked only `if (!llamaEngine.isModelLoaded())`. Since `FridayApplication.llamaEngine` was already loaded from the first query and the background service kept the process alive, `loadModel()` was never called again, causing the phone to permanently execute the original cached model regardless of user selection.
        * Model Reload Fix: `LlamaEngine.kt` now tracks `loadedModelPath`, `AgentCore.kt` reloads whenever `loadedModelPath != modelManager.getLlmModelPath()`, and `FridayService.reloadModels()` actively calls `FridayApplication.llamaEngine.loadModel(path)`.
