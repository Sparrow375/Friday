...
- **GitHub Actions**: Generates and signs `app-debug.apk` on every push to main/master, deploying it directly to GitHub Releases.
  ...
  * *Updated (June 2026 - Part 14)*: Resolved ONNX model loading crash on the phone (unsupported IR version 10) by programmatically forcing the exported [wakeword.onnx](file:///f:/Avaneesh/projects/Friday/app/src/main/assets/wakeword.onnx)'s `ir_version` to `8` in [train_wakeword.py](file:///f:/Avaneesh/projects/Friday/scripts/train_wakeword.py). Freed the local LLM brain entirely from function/tool calling in [AgentCore.kt](file:///f:/Avaneesh/projects/Friday/app/src/main/java/com/friday/assistant/intelligence/AgentCore.kt) to use it strictly for chatting purposes with simple, concise system instructions. Added a dedicated `Chat` tab page inside `MainActivity.kt` with modern bubbles and a clean voice/text interface to circumvent Samsung overlay keyboard focus blocks, while simplifying the WindowManager floating overlay strictly to a voice-only panel. Modified `ModelManager.kt` to force copy overwrite if cached model size does not match asset size.
